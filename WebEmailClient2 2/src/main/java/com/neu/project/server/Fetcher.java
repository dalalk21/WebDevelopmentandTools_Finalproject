...
//        	break;
    ...
    private void loadEmailsFromDB() {
        ...
        try {
            if (connection != null) {
                String query = "SELECT userId, emailAccount,emailPassword,email_id  FROM EmailAccount order by email_id desc";
...
//		Folder starredFolder = null;
        try {
            ...
            if (emaildata.emailAccount.toLowerCase().indexOf("gmail") > 0) {
                store.connect("imap.gmail.com", emaildata.emailAccount, emaildata.emailPassword);
            } else if (emaildata.emailAccount.toLowerCase().indexOf("yahoo") > 0) {
                store.connect("imap.mail.yahoo.com", emaildata.emailAccount, emaildata.emailPassword);
            } else{
                store.connect("imap."+emaildata.emailAccount.toLowerCase().substring(emaildata.emailAccount.indexOf("@")+1), emaildata.emailAccount, emaildata.emailPassword);
            }
...
//			}
    ...
    public void getEmailsAsynchronous(final EMailData emaildata){
