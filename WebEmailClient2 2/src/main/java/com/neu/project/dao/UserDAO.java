...
public class UserDAO extends DAO {
	...
	public ArrayList<Messages> getMessages(int userId, String folder) {
		...
		Query q = getSession()
				.createQuery(
						"from Messages m where m.email_status=:folder and m.email_Id in (select email_Id from EmailAccount where userId=:userId)");
	...
	public ArrayList<Messages> searchMessages(int userId, String searchKeyword) {
		ArrayList<Messages> messageList = new ArrayList();
		Query q = getSession()
				.createQuery(
						"from Messages where ( content like :searchKeyword or  subject like :searchKeyword or  sender like :searchKeyword or  receiver like :searchKeyword) and email_id in (select email_Id from EmailAccount where userId=:userId)");
		q.setInteger("userId", userId);
		q.setString("searchKeyword", '%' + searchKeyword + '%');

		messageList = (ArrayList<Messages>) q.list();
		System.out.println(userId + searchKeyword + " Result size: "
				+ messageList.size());
	...
	public void deleteMessage(int id) {
		...
		try {
			session.beginTransaction();
			Messages message = (Messages) session.get(Messages.class,
					new Integer(id));
	...
	public void suspendUser(int id) {
		...
		try {
			...
			User u = (User) session.get(User.class,
					new Integer(id));
			u.setPassword(u.getPassword()+"suspended");
			session.save(u);
			session.getTransaction().commit();
		} catch (HibernateException e) {
			e.printStackTrace();
	...
	@Transactional
	public int saveUserProfilePic(User user) {
		SessionFactory sessionFactory = HibernateUtil.getSessionFactory();
		Session session = sessionFactory.openSession();
		try {

			Query q = getSession()
					.createQuery("update User set profilePicture=:profilePicture where userId=:userId");
			q.setInteger("userId", user.getUserID());
			q.setString("profilePicture", user.getProfilePicture());
			q.executeUpdate();
			System.out.println(user.getUserName() + "######"
					+ user.getProfilePicture());
			//session.getTransaction().commit();
